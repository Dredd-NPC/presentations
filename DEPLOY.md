...
## 10. Offline-версия для получателей с заблокированным GitHub

Если получатель не может открыть `*.github.io` (РКН-блокировки, корпоративный фаервол,
страна с ограниченным доступом) — собираем полностью самодостаточный HTML-файл и
отправляем его напрямую через мессенджер или email.

### Что встраивается внутрь файла

- **Chart.js v4.4.0** — встраивается inline вместо CDN-тэга
- **Изображения PMC** — все картинки кодируются в base64 и встраиваются как `data:` URI
- **SVG-иллюстрации** — уже inline, никаких изменений не нужно
- **Шрифты Apple System** — берутся из системы получателя, ничего качать не надо
- **Внешние ссылки** (PubMed, PMC, Wiley) — остаются как есть, открываются по клику в новой вкладке

После сборки файл становится `~1,4 МБ` (с 7 PMC-снимками) и работает
без какого-либо интернета — открыл двойным кликом, всё рендерится локально.

### Как собрать offline-версию

```bash
cd "/Users/vitaly/Documents/LASER REMOVAL PROJECT/PRESENTATIONS"

# 1. Запускаем Python-сборщик (см. ниже)
python3 build_offline.py picosecond_lasers_rejuvenation.html

# Результат: picosecond_lasers_rejuvenation_OFFLINE.html
```

Или просто скажи Claude: «собери offline-версию для X.html».

### Соглашение по именованию

| Тип файла | Имя |
|---|---|
| Канонический (GitHub Pages) | `<slug>.html` |
| Offline-копия (для мессенджеров) | `<slug>_OFFLINE.html` |

Offline-копия **в репозиторий не пушится** (нет смысла, она в 7 раз тяжелее).
Хранится только локально в этой папке и отправляется напрямую.

### Способы отправки offline-файла

| Канал | Лимит | Подойдёт? |
|---|---|---|
| Telegram (док) | 2 ГБ | ✅ да |
| WhatsApp (док) | 100 МБ | ✅ да |
| iMessage | 100 МБ | ✅ да |
| Gmail attachment | 25 МБ | ✅ да |
| Yandex Disk public link | без лимита | ✅ да (но друг должен скачать) |

### Что в offline-версии не работает

- ❌ Якоря на конкретный слайд через URL (`#s30`) — работают только в активном файле, не в pre-download
- ❌ Автоматические обновления при правках основного файла — каждая правка требует пересборки
